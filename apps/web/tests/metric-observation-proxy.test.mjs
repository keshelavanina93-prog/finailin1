...
test("metric observation proxy forwards exact evidence requests and immutable reads only",async()=>{
 ...
 const body={metric:{resource_id:"metric",version_id:"version",content_hash:"b".repeat(64)},invocation_id:"invocation",expected_receipt_hash:"c".repeat(64),definition_snapshot:{valid_at:"2026-09-08T09:00:00Z",known_at:"2026-09-08T09:00:00Z"},valid_at:"2025-01-31T00:00:00.123456Z",known_at:"2026-09-08T08:00:00.654321Z"};
...
test("catalog preserves the exact Function filter and cursor without allowing writes",async()=>{
 const previous=globalThis.fetch,calls=[],query="?after_resource_id=cursor&function_resource_id=function&function_version_id=version&function_content_hash="+"a".repeat(64);
 globalThis.fetch=async(url,options)=>{calls.push({url,options});return Response.json({items:[]});};
 const context={params:Promise.resolve({path:["metrics"]})};
 function request(method,auth=true){const value=new Request("http://fixture.invalid/api/ontology/metrics"+query,{method,headers:auth?{authorization:"Bearer fixture"}:{}});value.nextUrl=new URL(value.url);return value;}
 try{
  assert.equal((await routes.GET(request("GET"),context)).status,200);
  assert.equal(new URL(calls[0].url).search,query);assert.equal(calls[0].options.cache,"no-store");
  assert.equal((await routes.POST(request("POST"),context)).status,404);
  assert.equal((await routes.GET(request("GET",false),context)).status,401);assert.equal(calls.length,1);
 }finally{globalThis.fetch=previous;}
});
