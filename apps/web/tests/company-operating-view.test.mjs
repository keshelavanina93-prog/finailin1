...
const {parseOperatingView,operatingViewPage,operatingViewSelection}=await import(`data:text/javascript;base64,${Buffer.from(ts.transpileModule(source,{compilerOptions:{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022}}).outputText).toString("base64")}`);
...
const view={version:1,...expected,group:"products",search:"Product",workSearch:"Review",workFilter:"PENDING_REVIEW",page:2,workPage:1,licencePage:3,selected:{resourceId:id,versionId:other}};
test("operating view restores exact company and microsecond snapshot with equivalent aware offsets",()=>{
 ...
 for(const change of [{companyId:other},{knownAt:"2026-09-08T12:00:00.123457Z"},{validAt:"2026-09-08T12:00:00"},{knownAt:"2026-02-30T12:00:00Z"},{version:2}])assert.equal(parseOperatingView(JSON.stringify({...view,...change}),expected),null);
});
test("preferences strip payloads and bound text, pages, group and work filters",()=>{
 ...
 assert.equal(parsed.group,"assets");assert.equal(parsed.workFilter,"ALL");assert.equal(parsed.search.length,200);assert.equal(parsed.workSearch,"");assert.equal(parsed.page,0);assert.equal(parsed.workPage,0);assert.equal(parsed.licencePage,499);assert.equal(parsed.selected,null);
});
