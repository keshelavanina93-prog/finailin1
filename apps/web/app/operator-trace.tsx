...
export default function OperatorTrace({token,root,onClose,onInspect}:{token:string;root:TraceSelection;onClose:()=>void;onInspect:(node:Node)=>void}) {
 ...
 const nodes=matching.slice(0,200);const levels=new Map<number,number>();
 ...
 return <section className="g8-trace" aria-label="System trace">
  ...
  <div className="g8-trace-toolbar"><label>Find in trace<input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Company, rule, source…"/></label><label><input type="checkbox" checked={technical} onChange={e=>setTechnical(e.target.checked)}/>Include schema mechanics</label><button onClick={()=>setZoom(z=>Math.max(.4,z-.2))} aria-label="Zoom out">−</button><span>{Math.round(zoom*100)}%</span><button onClick={()=>setZoom(z=>Math.min(2,z+.2))} aria-label="Zoom in">+</button><button onClick={()=>{setZoom(1);viewport.current?.scrollTo(0,0);}}>Reset canvas</button></div>
  ...
  {graph&&<><p>{nodes.length} of {graph.nodes.length} recorded versions visible{matching.length>200?" · narrow the filter to see additional matches":""}. Arrows point to dependencies. Drag the canvas or use its scrollbars.</p><div className="g8-trace-body">
   ...
   <aside aria-label="Trace object details">{selected&&<><small>{selected.object_type}</small><h3>{displayName(selected.display_name)}</h3><p>{selected.authority_state} · recorded {new Date(selected.system_from).toLocaleString()}</p><button onClick={()=>onInspect(selected)}>Open this version</button>{selected.source_document_id&&<button onClick={()=>void download(selected)}>Download original evidence</button>}<h4>Recorded connections</h4>{graph.edges.filter(e=>e.source_version_id===focus||e.target_version_id===focus).map(e=>{const other=graph.nodes.find(n=>n.version_id===(e.source_version_id===focus?e.target_version_id:e.source_version_id))!;return <button className="trace-connection" key={`${e.source_version_id}:${e.target_version_id}:${e.relation}`} onClick={()=>setFocus(other.version_id)}><small>{e.source_version_id===focus?"Depends on":"Used by"} · {e.relation.replace("FIELD:","")}</small>{displayName(other.display_name)}</button>;})}<details><summary>Identity and version</summary><p>{selected.resource_id}</p><p>{selected.version_id}</p></details></>}</aside>
  </div></>}
 </section>;
}
