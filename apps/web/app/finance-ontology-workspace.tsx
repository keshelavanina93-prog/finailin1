...
export default function FinanceOntologyWorkspace({ token, principal, onProposal }: { token: string; principal: Principal; onProposal: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        const [nextCatalog, nextConstructions] = await Promise.all([
          call<Catalog>("catalog", token), call<Construction[]>("constructions", token),
        ]);
        if (!cancelled) { setCatalog(nextCatalog); setConstructions(nextConstructions); }
      } catch (failure) {
        if (!cancelled) setError(failure instanceof Error ? failure.message : "Finance ontology unavailable");
      } finally {
