...
type Prepared = { filename: string; csv_text: string; context_version_id: string | null; codes: string[]; accounts: Account[] };
...
export default function EvidenceIntake({ token, principal, onRetained }: {
  ...
  async function prepare(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const csv_text = new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(await file.arrayBuffer());
      const context = await request<{ binding: { version_id: string } | null }>("/api/ontology/context");
      const context_version_id = sourceOnly ? null : context.binding?.version_id ?? null;
      const source = await request<{ account_codes: string[]; source_class: string }>("/api/ontology/context/source-accounts", {
        scope: principal.scope, filename: file.name, csv_text, context_version_id,
      });
      ...
      setPrepared({ filename: file.name, csv_text, context_version_id, codes: source.account_codes, accounts });
  ...
  return <section className="data-panel intake-binding">
    <form ref={fileForm} className="upload-strip" onSubmit={prepare}>
      <div><h3>Add evidence</h3><p>{principal.scope.legal_entity_id} · {principal.scope.period} · {principal.scope.currency}</p>
        <p>UTF-8 CSV · review shared account identities before retaining a financial construction.</p></div>
      <label>Source CSV<input type="file" accept=".csv,text/csv" name="source" required disabled={busy} onChange={() => setPrepared(null)} /></label>
    ...
    {prepared && <div className="binding-review"><h3>{prepared.filename}</h3>
      ...
      <button disabled={busy || (needsBindings && prepared.codes.some(code => !bindings[code]))} onClick={() => void retain()}>Retain & construct</button>
