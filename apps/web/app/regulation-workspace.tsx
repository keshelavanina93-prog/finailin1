...
type Result = {rules:Rule[];next_offset:number|null;run_id?:string;no_rules_found?:boolean;company?:{display_name:string};assessment_context?:{activity:string;at:string;known_at:string;customer_count:number|null}};
...
export default function RegulationWorkspace({token, companyId, onProposal}: {token:string;companyId:string;onProposal:(id:string)=>void}) {
  ...
  return <>
    <RegulatorySources token={token} onProposal={onProposal}/>
