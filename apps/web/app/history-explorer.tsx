...
type Search = {q:string;effective_at:string;known_at:string;offset:number};
function initialSearch(key:string):Search {
  const now=new Date().toISOString();
  const empty={q:"",effective_at:now,known_at:now,offset:0};
  try {const value=JSON.parse(sessionStorage.getItem(key)??"null");
    if(value&&typeof value.q==="string"&&value.q.length<=200&&typeof value.effective_at==="string"&&typeof value.known_at==="string"&&Number.isFinite(Date.parse(value.effective_at))&&Number.isFinite(Date.parse(value.known_at)))
      return {...empty,q:value.q,effective_at:value.effective_at,known_at:value.known_at};
...
export default function HistoryExplorer({token,companyId,companyName,contextKey,onInspect,onHistory,onTrace}:{
  ...
  onInspect:(resource:CanonicalResource)=>void;onHistory:(resource:CanonicalResource)=>void;onTrace:(resource:CanonicalResource)=>void;
}) {
  ...
  useEffect(()=>{
    ...
    void fetch(`/api/ontology/history-search?${params}`,{headers:{Authorization:`Bearer ${token}`},cache:"no-store",signal:controller.signal})
      .then(async response=>{const data=await response.json();if(!response.ok)throw Error(typeof data.detail==="string"?data.detail:"Recorded resources could not be loaded");if(!controller.signal.aborted){setResult(data);setError("");setLoading(false);}})
  ...
  function submit(event:FormEvent<HTMLFormElement>) {
    ...
    setSearch({q:query.trim(),effective_at:new Date(effective).toISOString(),known_at:new Date(known).toISOString(),offset:0});
  }
