...
import {assertProjectionTransport} from "./analysis-projection-identity";
...
export function sourceReviewContext(projection:AnalysisProjection|null,request:AnalysisRequest,state:{ready:boolean;busy:boolean;error:boolean;saved?:AnalysisView|null;excludedIndex:number|null;journalSnapshot?:string}):SourceReviewContext {
 const scope={companyId:request.company_id,invocationId:request.invocation_id,...(state.journalSnapshot===undefined?{}:{journalSnapshot:state.journalSnapshot})};
 if(!state.ready||state.busy)return {...scope,status:"updating"};
 if(state.error||!projection)return {...scope,status:"unavailable"};
 try{
  assertProjection(projection,request,state.saved);assertProjectionTransport(projection,state.journalSnapshot);
...
export function activeSourceReviewContext(value:SourceReviewContext|null,target:Scope|null):SourceReviewContext|null {
 ...
 return value?.companyId===target.companyId&&value.invocationId===target.invocationId&&value.journalSnapshot===target.journalSnapshot?value:{...target,status:"updating"};
}
...
export function explainSourceReview(value:SourceReviewContext):string {
 ...
 return `${value.title}.${value.journalSnapshot?` Accepted journals snapshot ${value.journalSnapshot}.`:""} Recorded authority: ${value.authority.replaceAll("_"," ").toLowerCase()}. Coverage: ${value.returnedRows} of ${value.totalRows} retained rows returned by the exact query; this is not complete company coverage. ${selected} ${basis} Effective ${value.validAt}; known ${value.knownAt}. ${value.reference?"Trace and History open the exact retained version at that knowledge cutoff.":"No exact resource reference is available for Trace or History."} These references do not establish a financial cause, performance conclusion or permission for business action.`;
}
