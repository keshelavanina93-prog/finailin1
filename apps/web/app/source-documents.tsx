...
export default function SourceDocuments({token,principal,onProposal,companyDocumentIds}:{companyDocumentIds?:string[];token:string;principal:Principal;onProposal:(id:string)=>void}) {
  ...
  function inspect(){void run(async signal=>{
    const data=await post(`/${reference}/companies/inspect`,JSON.stringify({mode,sheet,header_row:row,column}),signal);
    if(signal.aborted)return;setObserved(data);
  });}
  ...
  return <Panel title="Original source documents"><p>Retain the original workbook, then inspect its company evidence. Accounting interpretation and company review remain separate steps.</p>
    ...
    {observed&&<><table><thead><tr><th>Observed organization label</th><th>Observed cells</th><th>First source cell</th></tr></thead><tbody>{observed.companies.map(company=><tr key={company.source_label}><td>{company.source_label}</td><td>{company.row_count}</td><td>{company.first_coordinate}</td></tr>)}</tbody></table><p>{observed.unassigned_row_count} nonempty rows without a company label. Registration, group ownership, licences and chart applicability are not established by these labels.</p><p>These labels may identify companies, branches or regions. Keep them as source observations until their company or regional role is reviewed; labels alone cannot create a legal company.</p></>}
    ...
    <SourceLicenceNotice key={`licence:${reference}:${token}`} token={token} documentId={reference} canPropose={principal.permissions.includes("ontology_propose")} onProposal={onProposal}/>
