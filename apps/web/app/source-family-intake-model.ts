...
export const SOURCE_FAMILY_INTAKE_PLANS: readonly SourceFamilyIntakePlan[] = [
  ...
  },
  {
    ...
    implementedScopeDimensions: ["tenant", "legal_entity", "station", "currency", "period", "document"],
    requiredEvidence: [
      "source_file_hash",
      "schema_hash",
      "station_master",
    ...
    presentEvidence: ["source_file_hash", "schema_hash", "station_master"],
    routes: intakeRoutes(["/api/operations/*"], ["/v1/operations/*"], "planned"),
    currentImplementationStatus: "target-only",
    blockedOrMissingContracts: ["Forecourt shift-close, pump-meter and fuel-grade contracts are not implemented in the web app."],
  },
  {
    id: "gas_telemetry",
    label: "Gas telemetry",
    ...
    implementedScopeDimensions: ["tenant", "legal_entity", "period"],
    requiredEvidence: ["source_file_hash", "schema_hash", "row_count", "tank_level_reading", "external_reference", "lineage_receipt"],
    presentEvidence: ["source_file_hash"],
    routes: intakeRoutes(["/api/operations/*"], ["/v1/operations/*"], "planned"),
    currentImplementationStatus: "target-only",
    blockedOrMissingContracts: ["Telemetry grain, unit normalization and retained measurement receipts are missing."],
  },
  {
    ...
    implementedScopeDimensions: ["tenant", "legal_entity", "store", "currency", "period"],
    ...
    routes: intakeRoutes(["/api/hydration"], ["/v1/hydration/ingest"], "planned"),
    currentImplementationStatus: "target-only",
    blockedOrMissingContracts: ["Fiscal close evidence and cash-register identity matching are not yet typed."],
  },
  {
