...
function ChartFigure({ data, values }: { data: WorkspaceProjectionData; values: Point[] }) {
  ...
  if (chartType === "GANTT") {
    const intervals = (data.normalized_rows ?? []).filter(row => row.interval_start && row.interval_end).map(row => ({ label: row.labels.label ?? row.labels.period_id ?? row.row_id, start: Date.parse(row.interval_start as string), end: Date.parse(row.interval_end as string) })).filter(row => Number.isFinite(row.start) && Number.isFinite(row.end) && row.end >= row.start).slice(0, 24);
