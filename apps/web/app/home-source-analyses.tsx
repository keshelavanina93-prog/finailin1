...
function SourceAnalyses({token,companyId,onData,projectionGroup}:Props){
 ...
 function reviewRow(projection:AnalysisProjection,reference:HomeAnalysisReference,rowKey:string){try{const target=homeAnalysisRowTarget(projection,reference,companyId,rowKey);if(target)open(target);else setErrors(["No retained contributor is available for this row."]);}catch(failure){setErrors([failure instanceof Error?failure.message:"The exact row review is unavailable."]);}}
