...
export default function OperationalPromotionPreview({token, receiptId, profile}: {token: string; receiptId: string; profile?: string}) {
  ...
  const submit = () => { setSubmitting(true); setError(""); void operationsRequest<Proposal>(`petroleum/intake/${encodeURIComponent(receiptId)}/promotion-proposal`, token, undefined, {}).then(setProposal).catch(reason => setError(reason instanceof Error ? reason.message : "Proposal submission unavailable")).finally(() => setSubmitting(false)); };
  return <section className="source-detail" aria-label="Operational promotion preview"><h3>Governed promotion preview</h3><p><Badge tone={preview.candidates.length ? "good" : "warning"}>{preview.status}</Badge> This is a proposal packet only; no canonical mutation or accounting effect has occurred.</p>{!preview.candidates.length ? <Empty title="No eligible operational rows">Resolve grain, evidence, and accepted semantic bindings before preparing a governed proposal.</Empty> : <><div className="source-table"><table><thead><tr><th>Row</th><th>Candidate</th><th>Identity</th><th>Evidence</th></tr></thead><tbody>{preview.candidates.map(candidate => <tr key={`${candidate.object_type}:${candidate.source_row}`}><th scope="row">{candidate.source_row}</th><td>{candidate.object_type}</td><td>{candidate.identity_key}</td><td>{candidate.evidence.source_record_id}<br/><small>{candidate.evidence.source_hash}</small></td></tr>)}</tbody></table></div><button type="button" onClick={submit} disabled={submitting}>{submitting ? "Submitting governed proposal…" : "Submit for independent review"}</button></>}{proposal?.proposal?.proposal_id && <><p role="status">Proposal submitted: <code>{proposal.proposal.proposal_id}</code>. The retained proposal now enters the independent review gate.</p><PromotionReadiness token={token} proposalId={proposal.proposal.proposal_id} /></>}<p className="g8-context-note">Proposal required: {preview.proposal_required ? "yes" : "no"}. Canonical mutation: {preview.canonical_mutation ? "enabled" : "disabled"}.</p></section>;
}
