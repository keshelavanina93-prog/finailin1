...
export default function SemanticWorksheet({projection,fields,columns,layout,onLayout,onColumns,scroll,label,onPick}:Props){
 ...
 const returnedRows=projection.sections.flatMap(section=>section.row_keys.map(key=>({row:rowsByKey.get(key)!,section:section.label})));
 const rows=returnedRows.filter(({row})=>worksheetRowMatches(row,shown,layout.search));
 const hiddenSelection=Boolean(projection.request.selected_row&&!rows.some(({row})=>row.key===projection.request.selected_row));
 ...
 return <div className="semantic-worksheet" onPointerDownCapture={()=>navigationFocus.cancel()} onFocusCapture={event=>{if(!(event.target as HTMLElement).closest("[data-column]"))navigationFocus.cancel();}}>
  ...
  <div ref={scroll} className="semantic-grid" tabIndex={tabStop?undefined:0} onScroll={event=>{setTop(event.currentTarget.scrollTop);if(event.currentTarget.scrollLeft!==layout.left)onLayout({...layout,left:event.currentTarget.scrollLeft});}} aria-label="Retained analysis grid">
