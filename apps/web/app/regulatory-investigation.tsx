...
export default function RegulatoryInvestigation({token,companyId,assessment,onAssessment,viewStateKey,handoff,...navigation}:RegulatoryNavigation&{token:string;companyId:string;assessment:Result|null;onAssessment:()=>void;viewStateKey?:string;handoff?:CompanyRegulationHandoff}) {
 ...
 const [requestedCategory,setCategory]=useState<"rules"|"publications"|"monitors">(restored.category);
 const [workspaceLibrary,setWorkspaceLibrary]=useState(false);
 const category=workspaceLibrary?requestedCategory:"rules";
 ...
 useEffect(()=>{
  ...
  void Promise.allSettled([
   ...(category==="publications"?[publish(load<Publication>(`regulation/sources?offset=${offsets.publications}`,"publications"),setPublications)]:[]),
   ...(category==="rules"?[publish(loadRules(),value=>setRuleResponse({key:ruleKey,value}))]:[]),
   ...(category==="monitors"?[publish(load<Monitor>("regulation/monitors"),setMonitors)]:[]),
  ]);
  return()=>controller.abort();
 },[token,revision,companyId,offsets,snapshot,ruleKey,category]);
 const companyRules=rules?.rows.filter(row=>row.attributes.legal_entity_id===companyId)??[];
 const active=selected&&((category==="rules"&&selected.kind==="rule")||(category==="publications"&&selected.kind==="publication")||(category==="monitors"&&selected.kind==="monitor"))?selected:category==="rules"&&companyRules[0]?{kind:"rule",id:companyRules[0].resource_id}:null;
 ...
 function choose(kind:Selection["kind"],id:string){if(kind!=="rule"){setWorkspaceLibrary(true);setCategory(kind==="publication"?"publications":"monitors");}setSelected({kind,id});}
 function section(next:typeof category){setWorkspaceLibrary(next!=="rules");setCategory(next);setSelected(null);}
 ...
 return <section className="regi" aria-label="Regulatory investigation">
  <header className="regi-header"><div><p className="regi-eyebrow">REGULATORY INTELLIGENCE</p><h2>{workspaceLibrary?"Workspace legal library":"Company regulation & obligations"}</h2><p>Investigate what the retained evidence establishes, and what still needs review.</p></div><button onClick={()=>setRevision(value=>value+1)}><ClockCounterClockwise size={14}/>Refresh observations</button></header>
  <div className="regi-context"><span><ShieldCheck size={14}/>{workspaceLibrary?"All authorized workspace publications and monitors":"Reviewed interpretations for the selected company"}</span><span>{workspaceLibrary?"Library documents do not establish obligations for the selected company.":"Company applicability follows reviewed interpretations and their exact evidence."}</span></div>
  ...
  <nav className="regi-tabs" aria-label="Regulatory observation collections">{(workspaceLibrary?["rules","publications","monitors"] as const:["rules","publications"] as const).map(kind=><button key={kind} aria-pressed={category===kind} onClick={()=>section(kind)}>{kind==="rules"?"Company obligations":kind==="publications"?(workspaceLibrary?"Workspace publications":"Open workspace legal library"):"Workspace monitors"}</button>)}</nav>
