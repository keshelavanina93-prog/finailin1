...
export default function NyxInteraction({token,items,work,context,blockers,availability,onInspect,onData,onWork,mapSelection,inspection,onResourceTrace,onResourceHistory}:{token?:string;mapSelection?:MapSelection|null;inspection?:OperatorInspection|null;onResourceTrace?:ResourceNavigation;onResourceHistory?:ResourceNavigation;items:WorkItem[];work:WorkItem|null;context:string;blockers:string[];availability:string;onInspect:(item:WorkItem)=>void;onData:()=>void;onWork:()=>void}) {
 ...
 function materialContext(){
  ...
  if(state?.target_state==="CERTIFIED"){
   const receipt=lifecycle.certification;
   const claim=receipt?`Definition conformance certified. Retained proof: passed. ${receipt.proof.contract_attributes.definition.meaning} Limitations: ${receipt.proof.contract_attributes.definition.limitations}`:`A certified state is recorded, but its exact retained proof could not be verified. ${lifecycle.certificationError}`;
   return `${claim} This claim does not certify financial accuracy or source authenticity. How it is known: ${readable(state.epistemic_state)}; availability and quality: ${readable(state.availability_state)}; business lifecycle: ${readable(state.business_state)}. Historical state does not authorize current use.`;
  }
