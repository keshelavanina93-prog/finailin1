...
export type CompanyIndex = {source_companies:Node[];workspaces:{configuration:Node;company:Node;enterprise:Node;domain_pack:Node}[]};
...
export default function CompanyWorkspace({token,index,companyId,onSelect,onInspect}:{token:string;index:CompanyIndex|null;companyId:string;onSelect:(node:Node)=>void;onInspect:(node:Node)=>void}) {
 ...
 return <>
  <Panel title="Company structure">
   {groups.map(group=><section key={group.resource_id}><h3>{group.display_name}</h3><p>Configured enterprise workspace · corporate ownership is shown separately below.</p>
    {index?.workspaces.filter(w=>w.enterprise.resource_id===group.resource_id).map(w=><button key={w.configuration.resource_id} className="g8-panel-action" aria-pressed={companyId===w.company.resource_id} onClick={()=>onSelect(w.company)}>{w.configuration.display_name}<small>{w.domain_pack.display_name}</small></button>)}
   </section>)}
   {!groups.length&&<p>No company workspaces have been configured.</p>}
   <details><summary>Other company contexts with accounting sources</summary><p>Source-bound companies remain separate until their enterprise or operating relationships are established.</p>{index?.source_companies.filter(c=>!index.workspaces.some(w=>w.company.resource_id===c.resource_id)).map(c=><button className="g8-panel-action" key={c.resource_id} onClick={()=>onSelect(c)}>{c.display_name}</button>)}</details>
  </Panel>
  {busy&&<p role="status">Resolving company relationships and accounting context…</p>}{error&&<p role="alert">{error}</p>}
  {context&&<>
   <Panel title={context.company.display_name} aside={<Badge>Canonical company context</Badge>}>
    <p>Company ID: {context.company.resource_id}</p>
    <h3>Legal, operating and consolidation relationships</h3>
    {context.relationships.length?<table><thead><tr><th>From</th><th>Relationship</th><th>To</th><th>Evidence</th></tr></thead><tbody>{context.relationships.map(r=><tr key={r.record.resource_id}><td>{r.source.display_name}</td><td>{r.kind.replaceAll("_"," ")}</td><td>{r.target.display_name}</td><td><button onClick={()=>onInspect(r.record)}>Inspect version</button></td></tr>)}</tbody></table>:<p>No effective structural relationships are accepted for this company. Historical disclosures below do not create current ownership or consolidation membership.</p>}
    ...
    {!context.ledgers.length?<p>No accepted ledger is configured for this company. Accounting values and books cannot be inferred from the company name.</p>:<>
     <label>Company ledger<select value={ledgerId} onChange={e=>{setSelection(null);setLedgerId(e.target.value);setBookId("");setPeriodId("");}}><option value="">Select ledger</option>{context.ledgers.map(l=><option key={l.ledger.resource_id} value={l.ledger.resource_id}>{l.ledger.display_name}</option>)}</select></label>
     {ledger&&<><p>Chart: {ledger.chart_id?.display_name??"Unresolved"} · Calendar: {ledger.calendar_id?.display_name??"Unresolved"} · Currency: {ledger.currency_id?.display_name??"Unresolved"}</p>
      <label>Accounting book<select value={bookId} onChange={e=>{setSelection(null);setBookId(e.target.value);}}><option value="">Select book</option>{ledger.books.map(b=><option key={b.resource_id} value={b.resource_id}>{b.display_name}</option>)}</select></label>
      <label>Fiscal period<select value={periodId} onChange={e=>{setSelection(null);setPeriodId(e.target.value);}}><option value="">Select period</option>{ledger.periods.map(p=><option key={p.resource_id} value={p.resource_id}>{p.display_name}</option>)}</select></label>
    ...
    {context.dimensions.length?context.dimensions.map(d=><p key={d.resource_id}><button onClick={()=>onInspect(d)}>{d.display_name}</button> · {String(d.attributes.source_header)}</p>):<p>No source analytical dimension model is bound to this company.</p>}
    <h3>Company-bound source accounting</h3>
    {context.accounting_sources.length?context.accounting_sources.map(s=><section key={s.scope.resource_id}><button className="g8-link" onClick={()=>onInspect(s.scope)}>{s.scope.display_name}</button><p>{String(s.scope.attributes.worksheet)} · {String(s.scope.attributes.observed_from)} to {String(s.scope.attributes.observed_through)}</p>{s.bindings.length?s.bindings.map(b=><p key={b.resource_id}>{String(b.attributes.source_use).replaceAll("_"," ")} <button onClick={()=>onInspect(b)}>Inspect accounting binding</button></p>):<p>Accounting use awaits configuration.</p>}</section>):<p>No source accounting scope is bound to this company.</p>}
   </Panel>
   <Panel title="Reported company relationships">
    ...
    {context.disclosures.map(d=>{const o=d.observation.attributes.observation as {reported_role:string;reported_percent:string|null;former_indicator:string};return <div key={d.binding.resource_id}><strong>{String(d.binding.attributes.reporting_year)}</strong> · {d.reporter.display_name} reported <button className="g8-link" onClick={()=>onSelect(d.party)}>{d.party.display_name}</button> as {o.reported_role.toLowerCase()} · {o.reported_percent===null?"share not stated":`${o.reported_percent}%`}{o.former_indicator?` · former-party marker: ${o.former_indicator}`:""} <button onClick={()=>onInspect(d.binding)}>Source evidence</button></div>;})}
