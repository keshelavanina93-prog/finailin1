...
export default function CompanyWorkspace({token,index,companyId,onSelect,onInspect,onNavigate,onHistory,onTrace,onOperations,onMapSelection,viewStateKey,initialTab,onProposal,onWorkflow,canPropose=false,onInspectResource,onTraceResource,onJournalInspect,onJournalTrace}:{token:string;index:CompanyIndex|null;companyId:string;onSelect:(node:Node)=>void;onInspect:(node:Node,knownAt?:string)=>void;onNavigate?:(destination:CompanyDestination)=>void;onHistory?:(node:Node,knownAt?:string)=>void;onTrace?:(node:Node,knownAt?:string)=>void;onOperations:(state:MapWorkspaceState)=>void;onMapSelection:(selection:MapSelection|null)=>void;viewStateKey?:string;initialTab?:Tab;onProposal?:(id:string)=>void;onWorkflow?:(id:string)=>void;canPropose?:boolean;onJournalInspect?:(resource:Node,knownAt:string)=>void;onJournalTrace?:(resource:Node,knownAt:string)=>void}&SourceAccountNavigation) {
