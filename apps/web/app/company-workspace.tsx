...
export default function CompanyWorkspace({onContext,token,index,companyId,onSelect,onInspect,onNavigate,onRegulation,onHistory,onTrace,onOperations,onMapSelection,viewStateKey,initialTab,initialAccountingHandoff,onProposal,onWorkflow,canPropose=false,onInspectResource,onTraceResource,onJournalInspect,onJournalTrace}:{onContext?:(context:CompanyNyxContext)=>void;token:string;index:CompanyIndex|null;companyId:string;onSelect:(node:Node)=>void;onInspect:(node:Node,knownAt?:string)=>void;onNavigate?:(destination:CompanyDestination)=>void;onRegulation?:(handoff:CompanyRegulationHandoff)=>void;onHistory?:(node:Node,knownAt?:string)=>void;onTrace?:(node:Node,knownAt?:string)=>void;onOperations:(state:MapWorkspaceState)=>void;onMapSelection:(selection:MapSelection|null)=>void;viewStateKey?:string;initialTab?:Tab;initialAccountingHandoff?:CompanyAccountingHandoff;onProposal?:(id:string)=>void;onWorkflow?:(id:string)=>void;canPropose?:boolean;onJournalInspect?:(resource:Node,knownAt:string)=>void;onJournalTrace?:(resource:Node,knownAt:string)=>void}&SourceAccountNavigation) {
  ...
  <div className="c360-canvas">
   {!companyId?<div className="c360-empty c360-welcome"><Buildings size={34}/><p className="c360-eyebrow">COMPANY WORKSPACE</p><h2>Select the company behind your work</h2><p>Its accounting coverage, operating relationships and evidence will resolve from shared accepted resources.</p></div>:<>
    ...
    {context&&<div className="c360-content">
     {tab==="overview"&&<>{descriptor&&<CompanyCondition token={token} viewStateKey={viewStateKey} descriptor={descriptor} onLens={setTab} onInspect={inspect} onTrace={onTrace?trace:undefined} onHistory={onHistory?history:undefined} onProposal={onProposal} onWorkflow={onWorkflow} onSelect={pick} onData={()=>onNavigate?.("data")} onRegulation={()=>navigateCompany("regulation")} onWork={()=>onNavigate?.("workflows")} onOperations={onOperations} onMapSelection={onMapSelection}/>}<details className="c360-depth"><summary>Accounting readiness & recent retained changes</summary>
