...
export default function CompanyWorkspace({token,index,companyId,onSelect,onInspect,onNavigate,onHistory,onTrace,onOperations,onMapSelection,viewStateKey,initialTab,onProposal,canPropose=false,onInspectResource,onTraceResource,onJournalInspect,onJournalTrace}:{token:string;index:CompanyIndex|null;companyId:string;onSelect:(node:Node)=>void;onInspect:(node:Node,knownAt?:string)=>void;onNavigate?:(destination:CompanyDestination)=>void;onHistory?:(node:Node,knownAt?:string)=>void;onTrace?:(node:Node,knownAt?:string)=>void;onOperations:(state:MapWorkspaceState)=>void;onMapSelection:(selection:MapSelection|null)=>void;viewStateKey?:string;initialTab?:Tab;onProposal?:(id:string)=>void;canPropose?:boolean;onJournalInspect?:(resource:Node,knownAt:string)=>void;onJournalTrace?:(resource:Node,knownAt:string)=>void}&SourceAccountNavigation) {
 ...
 function inspect(node:Node){if(knownAt)onInspect(node,knownAt);}
 function trace(node:Node){if(knownAt)onTrace?.(node,knownAt);}
 function history(node:Node){if(knownAt)onHistory?.(node,knownAt);}
  ...
  <div className="c360-canvas">
   {!companyId?<div className="c360-empty c360-welcome"><Buildings size={34}/><p className="c360-eyebrow">COMPANY WORKSPACE</p><h2>Select the company behind your work</h2><p>Its accounting coverage, operating relationships and evidence will resolve from shared accepted resources.</p></div>:<>
    ...
    {context&&<div className="c360-content">
     {tab==="overview"&&<>{descriptor&&<CompanyCondition token={token} descriptor={descriptor} onLens={setTab} onInspect={inspect} onTrace={onTrace?trace:undefined} onSelect={pick} onData={()=>onNavigate?.("data")} onRegulation={()=>onNavigate?.("regulation")} onWork={()=>onNavigate?.("workflows")} onOperations={onOperations} onMapSelection={onMapSelection}/>}<details className="c360-depth"><summary>Accounting readiness & recent retained changes</summary>
