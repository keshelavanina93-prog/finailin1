...
export default function AccountingFacts({token}:{token:string}) {
  ...
  async function calculate(event:FormEvent<HTMLFormElement>) {
    ...
    try{
      ...
      const body=comparison?{left:query(left!),right:query(right!),as_of:date||null}:{query:query(choice),group_by:form.getAll("group"),as_of:date||null};
      const response=await fetch(`/api/ontology/model/facts/${selected}/${comparison?"reconcile":"aggregate"}`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(body)});
