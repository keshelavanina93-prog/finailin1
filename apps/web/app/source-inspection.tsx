...
export default function SourceInspection({ receipt, decision }: { receipt: IngestReceipt; decision?: ReviewDecision | null }) {
  ...
  return <section aria-label="Source analysis and persisted process">
    ...
    {profile?.account_catalogs?.map(catalog => <section key={catalog.sheet} aria-label="1C account definitions"><h3>1C chart of accounts · {catalog.account_count} source definitions</h3><p>{catalog.policy}</p><p>Company binding: {catalog.company_binding.replaceAll("_", " ")}</p><div className="data-scroll" style={{maxHeight: 480, overflow: "auto"}}><table><thead><tr><th>Account</th><th>Source name</th><th>Analytical dimensions</th><th>Source flags</th><th>Source coordinate</th></tr></thead><tbody>{catalog.accounts.map(account => <tr key={account.coordinate}><td>{account.account_code}</td><td>{account.source_name}</td><td>{account.analytics.map(item => <div key={item.position}>{item.position}. {item.source_label}</div>)}</td><td>Balance: {account.balance_behavior_source || "Unspecified"}<br/>Currency: {account.currency_tracking_source || "Unspecified"}<br/>Quantity: {account.quantity_tracking_source || "Unspecified"}</td><td>{account.coordinate}</td></tr>)}</tbody></table></div>{catalog.findings.map((finding, index) => <p key={index} role="status">{finding.code.replaceAll("_", " ")}: {finding.coordinates.join(", ")}</p>)}</section>)}
