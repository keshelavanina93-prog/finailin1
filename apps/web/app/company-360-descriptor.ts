...
const instant=(value:unknown):value is string=>typeof value==="string"&&/(Z|[+-]\d{2}:\d{2})$/i.test(value)&&Number.isFinite(Date.parse(value));
/** Retain the server's company snapshot; a response for another company cannot enter this view. */
export function companySnapshot(value:unknown,companyId:string):Snapshot {
