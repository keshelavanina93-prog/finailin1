...
import OperatingReport from "./operating-report";
...
export default function OperatorWorkspace({ token, principal, initialView = "intake", receiptId, proposalId }: {
  ...
  async function download(format: "source" | "export") {
    ...
    try {
      ...
      link.href = url; link.download = format === "source"
        ? (blob.type.includes("spreadsheetml") ? "retained-source.xlsx" : blob.type.includes("ms-excel") ? "retained-source.xls" : "retained-source.csv") : "g8-evidence-bundle.json";
  ...
  return <section className="engineering-workspace">
    <nav aria-label="Engineering tools" className="ontology-tabs">{([ ["intake", "Evidence intake"], ["objects", "Object workspace"], ["history", "Construction history"], ["ontology", "Registry & review"] ] as const).map(([id, label]) => <button key={id} className={view === id ? "active" : "quiet"} onClick={() => navigate(id)}>{label}</button>)}</nav>
      <div className="operator-content">{view === "ontology" ? <OntologyWorkspace token={token} principal={principal} initialProposalId={proposalId} onOpenObjectWorkspace={() => navigate("objects")} /> : <>{!detail && <div className="section-heading"><div><p className="overline">SOURCE → CONSTRUCTION → REVIEW → OBJECTS</p><h1>{view === "intake" ? "Evidence intake" : view === "objects" ? "Object workspace" : "Construction history"}</h1><p className="muted">{view === "objects" ? "Inspect accepted objects and follow every value back to source evidence." : "Review source construction without losing the evidence or earlier versions."}</p></div><button className="quiet" disabled={loading || busy} onClick={() => { setRevision(value => value + 1); }}>Refresh</button></div>}
        ...
        {!detail && view === "intake" && principal.permissions.includes("ingest") && <EvidenceIntake key={token} token={token} principal={principal} onRetained={async (receipt: IngestReceipt) => { setOffset(0); setRevision(value => value + 1); setNotice("Evidence retained with its reviewed identity selections."); await openConstruction(receipt.receipt_id); }} />}
        ...
        {detail?.receipt.source_profile?.sheets?.some(sheet => sheet.sheet === "Revenue Breakdown") && principal.permissions.includes("export") && principal.permissions.includes("ingest") && <OperatingReport key={`report:${detail.receipt.receipt_id}`} token={token} receiptId={detail.receipt.receipt_id} />}
        {detail && <ReceiptPanel key={detail.receipt.receipt_id} detail={detail} token={token} principal={principal} busy={busy} onDecision={decide} onExport={format => void download(format)} onClose={() => { selection.current++; setDetail(null); }} onObjects={() => { const id = detail.receipt.receipt_id; navigate("objects"); setVersion(id); }} />}
