...
function Workspace({token,companyId,invocationId,onInspect}:Props){
 ...
 useEffect(()=>{if(!projection||busy)return;if(scroll.current&&restoreScroll.current!==null){scroll.current.scrollTop=restoreScroll.current;restoreScroll.current=null;}if(returnFocus.current){const frame=requestAnimationFrame(()=>{if(lastButton.current?.isConnected)lastButton.current.focus({preventScroll:true});returnFocus.current=false;});return()=>cancelAnimationFrame(frame);}},[projection,busy]);
 ...
 return <section className="semantic-analysis" aria-label="Adaptive analysis workspace">
  ...
  {d&&projection&&<>
   ...
   <p className="semantic-time">Effective <time dateTime={d.valid_at}>{timeLabel(d.valid_at)}</time> · Known <time dateTime={d.known_at}>{timeLabel(d.known_at)}</time> · Retained <time dateTime={d.recorded_at}>{timeLabel(d.recorded_at)}</time></p>
