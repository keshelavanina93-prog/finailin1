"use client";
import {restoreCompanyWorkFocus,journalReviewEntryForSession,journalReviewOriginMatches,journalReviewHistoryMode,type CompanyWorkEntry,type CompanyWorkReference} from "./journal-review-handoff";
...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const [journalReviewEntry,setJournalReviewEntry]=useState<CompanyWorkEntry|null>(null);
  ...
  const companyContext=journalForeground&&!journalOriginVerified?null:displayedCompanyContext;
  ...
  useEffect(()=>{
    let frame=0;
    const refreshOrigin=()=>{if(!journalEntry)return;const reference=journalEntry.reference;window.dispatchEvent(new CustomEvent("g8:refresh-journal-review-origin",{detail:{companyId:reference.company.resource_id,validAt:reference.validAt,knownAt:reference.knownAt}}));};
    const back=()=>{
      ...
      if(mode==="review"){setJournalReviewMode("review");journalReturnPending.current=null;}
      else{setJournalReviewMode(null);journalReturnPending.current=journalEntry.entryId;frame=requestAnimationFrame(refreshOrigin);}
    };
    const settled=(event:Event)=>{
      const value=(event as CustomEvent).detail,origin=journalOrigin.current;
      if(!journalEntry||journalReturnPending.current!==journalEntry.entryId||origin?.entryId!==journalEntry.entryId||!journalOriginVerified||value?.companyId!==companyId||value.validAt!==journalEntry.reference.validAt||value.knownAt!==journalEntry.reference.knownAt)return;
      cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{
        if(journalReturnPending.current!==journalEntry.entryId)return;
        if(restoreCompanyWorkFocus(origin,document.getElementById("g8-main"),journalEntry.reference))journalReturnPending.current=null;
      });
    };
  ...
  },[journalEntry,journalOriginVerified,companyId]);
  ...
  function leaveJournalReview(){setJournalReviewMode(null);setJournalReviewEntry(null);journalOrigin.current=null;journalReturnPending.current=null;const state={...window.history.state};delete state.g8JournalReview;delete state.g8JournalReturn;window.history.replaceState(state,"",location.href);}
  function openJournalReview(reference:CompanyWorkReference){
    const element=businessSurface.current,queue=businessSurface.current?.querySelector<HTMLElement>("kind" in reference?"[data-company-work-queue]":"[data-journal-review-queue]");
    ...
    const entry:CompanyWorkEntry={token,entryId:crypto.randomUUID(),returnView:view as "home"|"companies",reference};
    ...
    journalReturnPending.current=null;setJournalReviewEntry(entry);setJournalReviewMode("review");clearSelection();setMenu(false);setSearch("");
  ...
  const clearSelection = () => {cancelResourceInspection();setDetailScope(companyId);setHistory(null);setTrace(null);setMapSelection(null);detailRequest.current++;setSelected(null);setWork(null);setReceipt(null);setProposal(null);setDetailError("");setDetailBusy(false);};
  ...
  async function inspect(resource:Pick<CanonicalResource,"resource_id">&Partial<Pick<CanonicalResource,"version_id"|"content_hash">>,pinnedVersion?:string,knownAt?:string) {
    ...
    try {
      ...
      else if(value.resource.resource_id!==resource.resource_id||(pinnedVersion&&value.resource.version_id!==pinnedVersion)||(knownAt&&restorationInstant(value.known_at)!==restorationInstant(knownAt)))throw Error("Inspection did not match the selected resource, version and knowledge cutoff.");
  ...
  return <SourceReviewNavigation.Provider value={openSourceReview}><div className={`g8-app ${sourceReview?"source-review-active":""} ${menu ? "menu-open" : ""} ${rail ? "rail-open" : ""} ${navFolded ? "nav-folded" : ""} ${nyxFolded ? "nyx-folded" : ""}`} style={{"--nyx-width":`${panelWidth}px`} as CSSProperties}>{accountPolicyTarget?.companyId===companyId&&<AccountDimensionPolicyWorkbench token={token} companyId={companyId} account={accountPolicyTarget.account} canPropose={principal.permissions.includes("ontology_propose")} onProposal={openBusinessProposal} onClose={()=>setAccountPolicyTarget(null)} onInspectResource={reference=>void inspect(reference,reference.version_id)} onTraceResource={reference=>setTrace({...reference,company_id:companyId})}/>}<a className="g8-skip" href="#g8-main">Skip to workspace</a>
    ...
    <div className="g8-work-canvas"><main id="g8-main" className="g8-main" data-view={sourceReview?"source-review":journalForeground?"actions":view}>{sourceReview&&analysisTarget&&<section className="g8-source-review-route" aria-label="Source review workspace"><div className="g8-source-review-nav"><button className="g8-link" onClick={returnFromSourceReview}>Back to {reviewOrigin?(reviewOrigin.view==="system"?"System / Engineering":navigation.find(area=>area.id===reviewOrigin.view)?.label??"Data & evidence"):"Data & evidence"}</button><span>Source review</span></div><div className="g8-source-review-content" hidden={Boolean(trace||history)}><SemanticAnalysisWorkspace onContext={onSourceContext} key={`${analysisTarget.companyId}:${analysisTarget.invocationId}:${analysisTarget.journalSnapshot??"source"}`} token={token} companyId={analysisTarget.companyId} invocationId={analysisTarget.invocationId} journalSnapshot={analysisTarget.journalSnapshot} onInspect={reference=>void inspect(reference,reference.version_id,reference.known_at)}/></div>{(trace||history)&&<div className="g8-source-review-inspector">{trace?tracePanel:historyPanel}</div>}</section>}{journalForeground&&<section data-journal-review-panel aria-label="Selected company work"><div hidden={Boolean(trace||history)&&journalOriginVerified}>{journalReviewMode==="review"&&journalEntry&&journalOriginVerified?<><p className="g8-context-note">Return context: {journalEntry.reference.company.display_name} - effective {journalEntry.reference.validAt}, known {journalEntry.reference.knownAt}. {"kind" in journalEntry.reference?`The originating work queue was observed ${journalEntry.reference.observedAt}. Workflow and proposal states below are current readback, separate from this company snapshot.`:"Proposal decisions below are current canonical review state, separate from this company snapshot."}</p><ActionWorkbench key={`${token}:${journalEntry.entryId}`} token={token} principal={principal} companyId={companyId} initialProposalId={"kind" in journalEntry.reference?undefined:journalEntry.reference.proposalId} initialWorkflowId={"kind" in journalEntry.reference?journalEntry.reference.workflowId:undefined} companyWorkflow={"kind" in journalEntry.reference?journalEntry.reference:undefined} onReturnFromWorkflow={returnJournalReview} onReturnFromProposal={returnJournalReview} onInspect={id=>void inspect({resource_id:id})}/></>:<div role="alert"><h2>Company work reference unavailable</h2><p>The original company, session or mounted snapshot no longer matches. No remembered work or latest company snapshot has been substituted.</p><button onClick={()=>navigate("home")}>Open workspace explicitly</button></div>}</div>{journalOriginVerified&&(trace||history)&&<div className="g8-source-review-inspector">{trace?tracePanel:historyPanel}</div>}</section>}<div ref={businessSurface} hidden={sourceReview||journalForeground} inert={journalForeground} style={{display:sourceReview||journalForeground?"none":"contents"}}>{(!sourceReview||reviewOrigin)&&<><div className="g8-breadcrumb">Workspace< CaretRight size={12}/>{view === "system" ? "System / Engineering" : navigation.find(area=>area.id===view)?.label}<span>{loading ? "Refreshing…" : `Updated ${updated}`}</span></div>
