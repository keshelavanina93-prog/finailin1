...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const foregroundView=companyMap?"operations":companyRegulation?"regulation":"actions";
  ...
  function openJournalReview(reference:CompanyForegroundEntry["reference"]){
    const element=businessSurface.current,queue=businessSurface.current?.querySelector<HTMLElement>(isCompanyMapHandoff(reference)?"[data-company-map-origin]":isCompanyRegulationOrigin(reference)?"[data-company-regulation-origin]":"kind" in reference?"[data-company-work-queue]":"[data-journal-review-queue]");
