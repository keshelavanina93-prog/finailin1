"use client";
import {restoreJournalReviewFocus,journalReviewEntryForSession,journalReviewOriginMatches,journalReviewHistoryMode,type JournalReviewEntry,type JournalReviewReference} from "./journal-review-handoff";
...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const [journalReviewEntry,setJournalReviewEntry]=useState<JournalReviewEntry|null>(null);
  ...
  useEffect(()=>{
    ...
    const settled=(event:Event)=>{
      ...
      cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{
        ...
        if(restoreJournalReviewFocus(origin,document.getElementById("g8-main"),journalEntry.reference.proposalId))journalReturnPending.current=null;
      });
    };
  ...
  function openJournalReview(reference:JournalReviewReference){
    const element=businessSurface.current,queue=businessSurface.current?.querySelector<HTMLElement>("[data-journal-review-queue]");
    ...
    const entry:JournalReviewEntry={token,entryId:crypto.randomUUID(),returnView:view as "home"|"companies",reference};
  ...
  return <SourceReviewNavigation.Provider value={openSourceReview}><div className={`g8-app ${sourceReview?"source-review-active":""} ${menu ? "menu-open" : ""} ${rail ? "rail-open" : ""} ${navFolded ? "nav-folded" : ""} ${nyxFolded ? "nyx-folded" : ""}`} style={{"--nyx-width":`${panelWidth}px`} as CSSProperties}>{accountPolicyTarget?.companyId===companyId&&<AccountDimensionPolicyWorkbench token={token} companyId={companyId} account={accountPolicyTarget.account} canPropose={principal.permissions.includes("ontology_propose")} onProposal={openBusinessProposal} onClose={()=>setAccountPolicyTarget(null)} onInspectResource={reference=>void inspect(reference,reference.version_id)} onTraceResource={reference=>setTrace({...reference,company_id:companyId})}/>}<a className="g8-skip" href="#g8-main">Skip to workspace</a>
    ...
    <main id="g8-main" className="g8-main" data-view={sourceReview?"source-review":journalForeground?"actions":view}>{sourceReview&&analysisTarget&&<section className="g8-source-review-route" aria-label="Source review workspace"><div className="g8-source-review-nav"><button className="g8-link" onClick={returnFromSourceReview}>Back to {reviewOrigin?(reviewOrigin.view==="system"?"System / Engineering":navigation.find(area=>area.id===reviewOrigin.view)?.label??"Data & evidence"):"Data & evidence"}</button><span>Source review</span></div><div className="g8-source-review-content" hidden={Boolean(trace||history)}><SemanticAnalysisWorkspace onContext={onSourceContext} key={`${analysisTarget.companyId}:${analysisTarget.invocationId}:${analysisTarget.journalSnapshot??"source"}`} token={token} companyId={analysisTarget.companyId} invocationId={analysisTarget.invocationId} journalSnapshot={analysisTarget.journalSnapshot} onInspect={reference=>void inspect(reference,reference.version_id,reference.known_at)}/></div>{(trace||history)&&<div className="g8-source-review-inspector">{trace?tracePanel:historyPanel}</div>}</section>}{journalForeground&&<section data-journal-review-panel aria-label="Journal proposal review"><div hidden={Boolean(trace||history)&&journalOriginVerified}>{journalReviewMode==="review"&&journalEntry&&journalOriginVerified?<><p className="g8-context-note">Return context: {journalEntry.reference.company.display_name} - effective {journalEntry.reference.validAt}, known {journalEntry.reference.knownAt}. Proposal decisions below are current canonical review state, separate from this company snapshot.</p><ActionWorkbench key={`${token}:${journalEntry.entryId}`} token={token} principal={principal} companyId={companyId} initialProposalId={journalEntry.reference.proposalId} onReturnFromProposal={returnJournalReview} onInspect={id=>void inspect({resource_id:id})}/></>:<div role="alert"><h2>Journal review reference unavailable</h2><p>The original company, session or mounted snapshot no longer matches. No remembered proposal or latest company snapshot has been substituted.</p><button onClick={()=>navigate("home")}>Open workspace explicitly</button></div>}</div>{journalOriginVerified&&(trace||history)&&<div className="g8-source-review-inspector">{trace?tracePanel:historyPanel}</div>}</section>}<div ref={businessSurface} hidden={sourceReview||journalForeground} inert={journalForeground} style={{display:sourceReview||journalForeground?"none":"contents"}}>{(!sourceReview||reviewOrigin)&&<><div className="g8-breadcrumb">Workspace< CaretRight size={12}/>{view === "system" ? "System / Engineering" : navigation.find(area=>area.id===view)?.label}<span>{loading ? "Refreshing…" : `Updated ${updated}`}</span></div>
