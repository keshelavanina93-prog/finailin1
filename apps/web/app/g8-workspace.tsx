...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  function openSourceReview(target:SourceReviewTarget){clearSelection();setCompanyId(target.companyId);setMenu(false);setSearch("");const url=new URL(location.href);url.pathname=sourceReviewPath(target);url.searchParams.delete("analysis_view");window.history.pushState(null,"",url);}
