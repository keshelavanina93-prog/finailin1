...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  return <div className={`g8-app ${menu ? "menu-open" : ""} ${rail ? "rail-open" : ""} ${navFolded ? "nav-folded" : ""} ${nyxFolded ? "nyx-folded" : ""}`} style={{"--nyx-width":`${panelWidth}px`} as CSSProperties}><a className="g8-skip" href="#g8-main">Skip to workspace</a>
    ...
    {view === "system" ? <><div className="g8-page-heading"><div><p className="overline">ADVANCED INTERNAL ACCESS</p><h1>System / Engineering</h1><p>Evidence intake, construction, registry and governed review tools.</p></div><Badge>Advanced</Badge></div><Engineering key={`${engineering.view}:${engineering.receiptId ?? ""}:${engineering.proposalId ?? ""}`} token={token} principal={principal} initialView={engineering.view} receiptId={engineering.receiptId} proposalId={engineering.proposalId}/></> : <>
      ...
      {view === "actions" && <ActionWorkbench key={`${token}:${companyId}`} token={token} principal={principal} companyId={companyId} onInspect={id=>void inspect({resource_id:id})}/>}
      ...
      {view === "ontology" && ontologySection==="accounting" && <AccountingFacts key={`${companyId}:${selected?.resource.version_id??"analysis"}`} token={token} authorityConsumer={typeof selected?.resource.attributes.minimum_authority_state==="string"?selected.resource:undefined} />}
      {view === "ontology" && ontologySection==="sets" && <ObjectSets key={companyId} token={token} onProposal={id => openEngineering("ontology", undefined, id)} />}
      ...
      {history&&<OperatorHistory key={history.resource_id} token={token} selection={history} onClose={()=>setHistory(null)} onSelect={version=>{setHistory({...history,version_id:version.version_id});void inspect(version,version.version_id);}} onTrace={version=>setTrace({resource_id:version.resource_id,version_id:version.version_id,company_id:companyId})} onInspect={id=>void inspect({resource_id:id})}/>}
