...
type View = "home" | "companies" | "data" | "ontology" | "system" | "operations" | "regulation";
...
const navigation = [
 ...
 {id:"actions",label:"Workflows & Actions",hint:undefined,icon:FlowArrow,available:false},
];
...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const clearSelection = () => {setMapSelection(null);detailRequest.current++;setSelected(null);setWork(null);setReceipt(null);setProposal(null);setDetailError("");setDetailBusy(false);};
  async function inspect(resource:CanonicalResource) {setNyxTab("context");setMapSelection(null);
  ...
  useEffect(() => {
    if (loading || initialSelection.current || !items.length) return;
    initialSelection.current = true;
    void inspectWork(items.find(item => item.state === "PENDING") ?? items[0], false);
  }, [loading, items, inspectWork]);
  ...
  return <div className={`g8-app ${menu ? "menu-open" : ""} ${rail ? "rail-open" : ""} ${navFolded ? "nav-folded" : ""} ${nyxFolded ? "nyx-folded" : ""}`} style={{"--nyx-width":`${panelWidth}px`} as CSSProperties}><a className="g8-skip" href="#g8-main">Skip to workspace</a>
    ...
    {view === "system" ? <><div className="g8-page-heading"><div><p className="overline">ADVANCED INTERNAL ACCESS</p><h1>System / Engineering</h1><p>Evidence intake, construction, registry and governed review tools.</p></div><Badge>Advanced</Badge></div><Engineering key={`${engineering.view}:${engineering.receiptId ?? ""}:${engineering.proposalId ?? ""}`} token={token} principal={principal} initialView={engineering.view} receiptId={engineering.receiptId} proposalId={engineering.proposalId}/></> : <>
      ...
      <div className="g8-home-bottom"><div ref={workRef}><Panel title="My work" aside={<Badge>{loading?"Checking":`${pending.length} awaiting review`}</Badge>}>{workTable}</Panel></div><Panel title="Recent findings" aside={<button className="g8-link" onClick={()=>{setWorkFilter("all");workRef.current?.scrollIntoView({behavior:"smooth"});}}>View all</button>}>{[...items].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5).map(item=><Signal key={`${item.kind}:${item.id}`} title={item.title} detail={`${readable(item.state)} · ${date(item.date)} · ${item.reason}`} tone={item.state==="APPROVED"?"good":item.state==="REJECTED"?"bad":"warning"} onClick={()=>{void inspectWork(item);setNyxTab("context");}}/>)}{!items.length&&<Empty title={loading?"Loading findings…":"No retained findings"}>Findings will be drawn from your available source and review history.</Empty>}</Panel></div></>}
      {view === "regulation" && <RegulationWorkspace key={companyId} token={token} companyId={companyId} onProposal={id => openEngineering("ontology", undefined, id)} />}
      ...
      {view === "companies" && <><CompanySetup token={token} principal={principal} onSubmitted={value=>{refresh();void inspectWork({id:value.proposal.proposal_id,kind:"proposal",title:value.proposal.title,state:"PENDING",reason:value.proposal.rationale,date:value.created_at,severity:1});}}/><Panel title="Company directory" aside={<Badge>{companies.length} available</Badge>}>{snapshot.graph.error ? <Empty title="Company directory unavailable">{snapshot.graph.error}</Empty> : !companies.length ? <Empty title={loading ? "Loading companies…" : "No accepted company identities"}>Propose your company above. Once independently approved, it becomes available here; evidence binding remains a separate governed step.</Empty> : <div className="g8-company-list">{companies.map(item=><article key={item.resource_id}><Buildings size={28}/><div><h3>{item.display_name}</h3><p>{readable(item.object_type)} · {readable(item.evidence_class)}</p><Badge tone={tone(item.authority_state)}>{readable(item.authority_state)}</Badge></div><button className="g8-link" onClick={()=>{setCompanyId(item.resource_id);clearSelection();navigate("home");}}>Open workspace<ArrowRight size={16}/></button><button className="g8-icon" aria-label={`Inspect ${item.display_name}`} onClick={()=>void inspect(item)}><GraphIcon size={19}/></button></article>)}</div>}</Panel></>}
      {view === "data" && <>{snapshot.evidence.error ? <Empty title="Source inventory unavailable">{snapshot.evidence.error}</Empty> : <SourceExplorer key={currentCompany?.resource_id ?? "scope"} token={token} principal={principal} sources={scopedEvidence} onSelect={inspectSource} initialReceiptId={work?.kind === "evidence" ? work.id : undefined} onReview={id=>openEngineering("history",id)}/>}<div className="g8-panel-foot">Evidence scope: {boundCompany?.display_name ?? "Sign-in scope"} · {principal.scope.period} · {principal.scope.currency}<button className="g8-link" onClick={()=>openEngineering("intake")}>Retain a new source<ArrowRight size={13}/></button></div></>}
