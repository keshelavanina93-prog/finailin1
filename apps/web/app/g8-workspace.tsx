...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const [savedLayout]=useState(()=>{try {const value=JSON.parse(localStorage.getItem("g8-layout-v1")??"{}");return {dock:value.dock==="bottom"?"bottom":"right",nav:value.nav===true,nyx:value.nyx===true,width:typeof value.width==="number"&&Number.isFinite(value.width)?Math.max(300,Math.min(1600,value.width)):340};}catch{return {dock:"right",nav:false,nyx:false,width:340};}});
  ...
  useEffect(()=>{try{localStorage.setItem("g8-layout-v1",JSON.stringify({nav:navFolded,nyx:nyxFolded,width:nyxWidth,dock:nyxDock}));}catch{/* Layout persistence is optional. */}},[navFolded,nyxFolded,nyxWidth,nyxDock]);const [viewport,setViewport]=useState(1440);
