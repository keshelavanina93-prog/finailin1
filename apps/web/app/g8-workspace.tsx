...
import {sameCompanyMapSelection,companyMapHandoff,isCompanyMapHandoff,restoreCompanyMapFocus,type CompanyForegroundEntry} from "./company-map-handoff";
...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  ...
  const foregroundView=companyMap?"operations":"actions";
  ...
  useEffect(()=>{
    let frame=0;
    const refreshOrigin=()=>{if(!journalEntry||isCompanyMapHandoff(journalEntry.reference))return;const reference=journalEntry.reference;window.dispatchEvent(new CustomEvent("g8:refresh-journal-review-origin",{detail:{companyId:reference.company.resource_id,validAt:reference.validAt,knownAt:reference.knownAt}}));};
    const back=()=>{
      ...
      if(mode==="review"){setJournalReviewMode("review");journalReturnPending.current=null;if(isCompanyMapHandoff(journalEntry.reference)){clearSelection();setMapSelection(journalEntry.mapSelection===undefined?journalEntry.reference.selection:journalEntry.mapSelection);}}
      else{setJournalReviewMode(null);journalReturnPending.current=journalEntry.entryId;
        if(isCompanyMapHandoff(journalEntry.reference)){clearSelection();setMapSelection(journalEntry.reference.selection);const origin=journalOrigin.current;frame=requestAnimationFrame(()=>{if(journalReturnPending.current===journalEntry.entryId&&restoreCompanyMapFocus(origin,document.getElementById("g8-main")))journalReturnPending.current=null;});}
    ...
    const settled=(event:Event)=>{
      ...
      if(!journalEntry||isCompanyMapHandoff(journalEntry.reference)||journalReturnPending.current!==journalEntry.entryId||origin?.entryId!==journalEntry.entryId||!journalOriginVerified||value?.companyId!==companyId||value.validAt!==journalEntry.reference.validAt||value.knownAt!==journalEntry.reference.knownAt)return;
  ...
  function openJournalReview(reference:CompanyForegroundEntry["reference"]){
    const element=businessSurface.current,queue=businessSurface.current?.querySelector<HTMLElement>(isCompanyMapHandoff(reference)?"[data-company-map-origin]":"kind" in reference?"[data-company-work-queue]":"[data-journal-review-queue]");
