...
import FinanceOntologyWorkspace from "./finance-ontology-workspace";
...
function SignedIn({token,principal,onSignOut}: {token:string;principal:Principal;onSignOut:()=>void}) {
  const [linkedAnalysis,setLinkedAnalysis]=useState(()=>{try{const raw=new URL(location.href).searchParams.get("analysis_view");return raw?parseView(raw,JSON.parse(raw).request.company_id):null;}catch{return null;}});
  ...
  const [savedContext]=useState<{companyId:string;view:View;trace:TraceSelection|null;history:HistorySelection|null}>(()=>{
    if(linkedAnalysis)return {companyId:linkedAnalysis.request.company_id,view:"data" as View,trace:null,history:null};
  ...
  const [savedLayout]=useState(()=>{try {const value=JSON.parse(localStorage.getItem("g8-layout-v1")??"{}");return {height:typeof value.height==="number"&&Number.isFinite(value.height)?Math.max(180,Math.min(700,value.height)):300,dock:value.dock==="bottom"?"bottom":"right",nav:value.nav===true,nyx:value.nyx===true,width:typeof value.width==="number"&&Number.isFinite(value.width)?Math.max(300,Math.min(1600,value.width)):340};}catch{return {height:300,dock:"right",nav:false,nyx:false,width:340};}});
  const [navFolded,setNavFolded]=useState(savedLayout.nav);const [nyxFolded,setNyxFolded]=useState(savedLayout.nyx);
  const [nyxHeight,setNyxHeight]=useState(savedLayout.height);const [viewportHeight,setViewportHeight]=useState(900);
  const [nyxWidth,setNyxWidth]=useState(savedLayout.width);const [nyxDock,setNyxDock]=useState(savedLayout.dock);
  useEffect(()=>{try{localStorage.setItem("g8-layout-v1",JSON.stringify({nav:navFolded,nyx:nyxFolded,width:nyxWidth,dock:nyxDock,height:nyxHeight}));}catch{/* Layout persistence is optional. */}},[navFolded,nyxFolded,nyxWidth,nyxDock,nyxHeight]);const [viewport,setViewport]=useState(1440);
  const [nyxTab,setNyxTab]=useState("interact");
  const drag=useRef<{x:number;width:number}|null>(null);
  useEffect(()=>{const resize=()=>{setViewport(window.innerWidth);setViewportHeight(window.innerHeight);};resize();window.addEventListener("resize",resize);return()=>window.removeEventListener("resize",resize);},[]);
  ...
  const bottomNyx=nyxDock==="bottom"||viewport<=1000;const maxNyxHeight=Math.max(180,Math.floor(viewportHeight*.42));const maxNyxSize=bottomNyx?maxNyxHeight:maxNyxWidth;const minNyxSize=bottomNyx?180:300;const currentNyxSize=bottomNyx?Math.min(nyxHeight,maxNyxHeight):panelWidth;
  function resizeNyx(value:number){const size=Math.max(minNyxSize,Math.min(maxNyxSize,value));if(bottomNyx)setNyxHeight(size);else setNyxWidth(size);}
  ...
  useEffect(()=>{const frame=requestAnimationFrame(()=>window.scrollTo({top:viewScroll.current[`${companyId}:${view}`]??0,behavior:"instant"}));return()=>cancelAnimationFrame(frame);},[view,companyId]);
  ...
  const navigate = (next:View) => {viewScroll.current[`${companyId}:${view}`]=window.scrollY;if(next==="finance")setFinanceVisited(true);setCompanyInitialTab(undefined);setView(next);setTrace(null);setMenu(false);setSearch("");};
  ...
  return <div className={`g8-app ${nyxDock==="bottom"?"nyx-bottom":""} ${menu ? "menu-open" : ""} ${rail ? "rail-open" : ""} ${navFolded ? "nav-folded" : ""} ${nyxFolded ? "nyx-folded" : ""}`} style={{"--nyx-width":`${panelWidth}px`,"--nyx-height":`${Math.min(nyxHeight,maxNyxHeight)}px`} as CSSProperties}>{accountPolicyTarget?.companyId===companyId&&<AccountDimensionPolicyWorkbench token={token} companyId={companyId} account={accountPolicyTarget.account} canPropose={principal.permissions.includes("ontology_propose")} onProposal={openBusinessProposal} onClose={()=>setAccountPolicyTarget(null)} onInspectResource={reference=>void inspect(reference,reference.version_id)} onTraceResource={reference=>setTrace({...reference,company_id:companyId})}/>}<a className="g8-skip" href="#g8-main">Skip to workspace</a>
    ...
    <main id="g8-main" className="g8-main" data-view={view}><div className="g8-breadcrumb">Workspace< CaretRight size={12}/>{view === "system" ? "System / Engineering" : navigation.find(area=>area.id===view)?.label}<span>{loading ? "Refreshing…" : `Updated ${updated}`}</span></div>
    ...
    {view === "system" ? <><div className="g8-page-heading"><div><p className="overline">ADVANCED INTERNAL ACCESS</p><h1>System / Engineering</h1><p>Evidence intake, construction, registry and governed review tools.</p></div><Badge>Advanced</Badge></div><Engineering key={`${engineering.view}:${engineering.receiptId ?? ""}:${engineering.proposalId ?? ""}`} token={token} principal={principal} initialView={engineering.view} receiptId={engineering.receiptId} proposalId={engineering.proposalId}/></> : <>
      ...
      {view === "operations" && <OperationsMap key={currentCompany?.resource_id??"scope"} {...mapProps}/>}
       {view === "ontology" && <><nav className="g8-module-tabs" aria-label="Ontology views">{[["resources","Business resources"],["sets","Account analysis"],["accounting","Accounting contracts"],["finance","Finance ontology"]].map(([id,title])=><button key={id} aria-pressed={ontologySection===id} onClick={()=>setOntologySection(id)}>{title}</button>)}</nav>{ontologySection==="accounting"&&<p className="g8-subtle">Shared ontology definitions and queries cover your authorized scope. Company filtering is specified in each query or contract.</p>}</>}
      <div hidden={view!=="finance"}>{financeVisited&&<FinanceWorkspace onClearReport={()=>setFinanceTarget(null)} initialReport={financeTarget} key={companyId} token={token} companyId={companyId} context={resolvedContext} contextError={companyContextError} onContext={()=>{navigate("companies");setCompanyInitialTab("accounting");}} onInspect={reference=>void inspect(reference,reference.version_id,reference.known_at)} onTrace={reference=>setTrace({...reference,company_id:companyId})}/>}</div>
       ...
       {view === "ontology" && ontologySection==="finance" && <FinanceOntologyWorkspace token={token} principal={principal} onProposal={openBusinessProposal} />}
