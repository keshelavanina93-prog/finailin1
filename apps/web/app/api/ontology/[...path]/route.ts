...
async function forward(request: NextRequest, context: Context) {
  ...
  const finance = (request.method === "GET" && /^finance\/(?:catalog|domain|dimensions\/policies|constructions|contracts|functions|actions|runs\/fcr_[a-f0-9]{64}(?:\/export)?|tb\/(?:sources|reconciliation|contract|runs\/fcr_[a-f0-9]{64}|diagnostics))$/.test(route)) || (request.method === "POST" && /^finance\/(?:catalog\/proposals|candidates\/(?:preview|proposals)|dimensions\/validate|classify|execute|journal-trial-balance|tb\/(?:draft|export|command))$/.test(route));
  ...
  const outcomes = ((request.method === "GET" && /^outcomes\/(?:actual-vs-plan|learning-evaluation|measurements|learning-candidates)$/.test(route)) || (request.method === "POST" && (/^outcomes\/learning-candidates$/.test(route) || /^outcomes\/learning-candidates\/lc_[a-f0-9]{64}\/decision$/.test(route) || route === "outcomes/measurements")));
  ...
  const petroleumReconciliation=request.method==="GET"&&/^operations\/petroleum\/(?:reconciliation|lineage\/[a-fA-F0-9-]+|intake\/[a-zA-Z0-9_-]{1,128}\/validation)$/.test(route);
