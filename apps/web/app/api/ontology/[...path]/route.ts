...
async function forward(request: NextRequest, context: Context) {
  ...
  if (!/^(catalog|context(?:\/(?:accounts|source-accounts))?|graph|aliases|reference-proposal|resources(?:\/[a-fA-F0-9-]+)?|resolve\/[a-fA-F0-9-]+|proposals(?:\/[a-fA-F0-9-]+(?:\/decision)?)?)$/.test(route)) {
