...
async function forward(request: NextRequest, context: Context) {
  ...
  const validation = (request.method === "GET" && /^external\/validation\/runs\/[a-fA-F0-9]{8}(?:-[a-fA-F0-9]{4}){3}-[a-fA-F0-9]{12}$/.test(route))
    || (request.method === "POST" && /^external\/validation\/runs\/[a-fA-F0-9]{8}(?:-[a-fA-F0-9]{4}){3}-[a-fA-F0-9]{12}\/report-proposals$/.test(route));
