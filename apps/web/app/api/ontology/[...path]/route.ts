...
async function forward(request: NextRequest, context: Context) {
  ...
  const model = /^model\/(?:fact-runs\/fcr_[a-f0-9]{64}|definitions(?:\/(?:preview|contracts|[a-fA-F0-9-]+))?|proposals\/[a-fA-F0-9-]+\/decision|(?:sets|groups)\/[a-fA-F0-9-]+\/objects|bindings\/[a-fA-F0-9-]+\/proposal|facts\/[a-fA-F0-9-]+\/(?:aggregate|reconcile)|sources\/ir_[a-f0-9]{64}\/accounts(?:\/proposal)?|derived\/query)$/.test(route);
  if (!/^operations(?:\/(?:licence-notices|bindings|opa_[a-f0-9]{64}(?:\/resume)?))?$/.test(route) && !documents && !/^regulation\/(rules|proposals|assessments(?:\/fcr_[a-f0-9]{64})?|sources(?:\/(?:capture|proposals|compare|inspect))?)$/.test(route) && !model && !lifecycle && !eventTime && route !== "company-context" && route !== "object-sets/query" && !/^(catalog|context(?:\/(?:accounts|source-accounts))?|graph|aliases|reference-proposal|rollback-proposal|resources(?:\/[a-fA-F0-9-]+(?:\/graph)?)?|resolve\/[a-fA-F0-9-]+|proposals(?:\/[a-fA-F0-9-]+(?:\/(?:decision|promotion-check))?)?)$/.test(route)) {
