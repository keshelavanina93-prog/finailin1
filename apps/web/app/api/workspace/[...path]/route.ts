...
async function forward(request: NextRequest, context: Context) {
  ...
  if (!/^(session|summary|intake|workflows(?:\/[a-zA-Z0-9_-]+(?:\/control)?)?|report-inputs|report-calculations(?:\/[a-zA-Z0-9_-]+(?:\/export)?)?|objects(?:\/[a-zA-Z0-9_:.-]+)?|constructions\/[a-zA-Z0-9_-]+(?:\/(decision|source|export|preview))?|diagnostic-targets|diagnostics|executable-preflight|executable-functions)$/.test(route)) {
