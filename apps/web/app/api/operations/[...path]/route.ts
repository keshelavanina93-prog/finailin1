...
async function forward(request:NextRequest,context:Context) {
 ...
 if(!/^(map(?:\/[a-fA-F0-9-]+\/connections)?|import-proposal|petroleum\/(?:reconciliation|margin|movement-journal-reconciliation|lineage\/[a-fA-F0-9-]+|intake\/[a-zA-Z0-9_-]{1,128}\/validation|intake\/[a-zA-Z0-9_-]{1,128}\/promotion-preview|intake\/[a-zA-Z0-9_-]{1,128}\/promotion-proposal))$/.test(route)) return Response.json({detail:"Operations route not found"},{status:404});
