...
export type PetroleumOutcome = {contract:"petroleum-outcome-measurement/1";status:"MEASURED_RESOLVED"|"MEASURED_UNRESOLVED";control_id:string;variance_id:string;scope:Record<string,string>;baseline:{variance_quantity:string;review_status:string;source_hashes:string[];time:PetroleumVariance["time"]};current:{variance_quantity:string;review_status:string;source_hashes:string[];time:PetroleumVariance["time"]};readback_id:string;measurement_hash:string;authority:{physical_outcome_measured:true;accounting_authorized:false;business_effect_authorized:false}};
export type PetroleumControl = {contract:"petroleum-control/1";control_id:string;variance:PetroleumVariance;state:"INVESTIGATION_OPEN"|"EXPLANATION_ACCEPTED"|"EXPLANATION_REJECTED"|"ACTION_PROPOSED"|"APPROVED"|"REFUSED"|"READBACK_VERIFIED";events:Array<Record<string,unknown>>;initiator_actor_id:string;execution:"REFUSED_NO_EXTERNAL_ADAPTER"|"LOCAL_READBACK_VERIFIED"|"EXTERNAL_READBACK_VERIFIED";readback:PetroleumActionReadback|null;outcome:PetroleumOutcome|null;accounting_authorized:false;business_effect_authorized:false};
