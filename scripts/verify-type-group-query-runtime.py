"""Verify a reviewed source-record type group over authentic retained observations."""
...
loader = importlib.util.spec_from_file_location(
    "type_group_proof_shared", ROOT / "scripts/verify-transformation-input-runtime.py"
)
shared = importlib.util.module_from_spec(loader)
loader.loader.exec_module(shared)
KEY = "retained-observations:source-record-type-group:v1"
CASES = {
...
def prepare(author, reviewer):
    group = shared.publish(
        author,
        reviewer,
        "ObjectTypeGroup",
        KEY,
        "Retained source account and movement observations",
        {
            "definition": {"types": [case["type"] for case in CASES.values()]},
        },
    )
    query = {
        "object_type": "ObjectTypeGroup",
        "resource_ids": sorted(CASES),
        "type_group": pin(group),
    }
    object_set = shared.publish(
        ...
        "ObjectSetDefinition",
        KEY,
        "Two retained source observations through their shared type group",
        {"definition": query},
    )
    ...
    function = shared.publish(
        ...
        "FunctionDefinition",
        KEY,
        "Read retained observations through their reviewed type group",
        {
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/development/evidence/nin6-type-group-query-runtime.json"),
    )
    ...
    with httpx.Client(
        ...
        bindings = result["type_group_bindings"]
        assert pin(bindings["group"]) == prepared["type_group"]
        assert {item["object_type"] for item in bindings["schemas"]} == {
            case["type"] for case in CASES.values()
        }
        assert bindings["fields"]["source_record_id"]["target_type"] == "SourceRecord"
        assert bindings["fields"]["evidence_id"]["target_type"] == "SourceEvidence"
        for item in bindings["schemas"]:
            case = next(
                case for case in CASES.values() if case["type"] == item["object_type"]
            )
            assert item["schema"]["resource_id"] == case["schema"]
        assert len(result["type_group_values"]) == 2
        for projected in result["type_group_values"]:
            ...
            schema = next(
                item["schema"]
                for item in bindings["schemas"]
                if item["object_type"] == obj["object_type"]
            )
            assert projected["schema_version_id"] == schema["version_id"]
        ...
        first_page = query({"limit": 1, "offset": 0})
        second_page = query({"limit": 1, "offset": 1})
        assert first_page["total"] == second_page["total"] == 2
        assert first_page["next_offset"] == 1 and second_page["next_offset"] is None
        assert first_page["objects"] + second_page["objects"] == result["objects"]
        assert (
            first_page["objects"][0]["resource_id"]
            != second_page["objects"][0]["resource_id"]
        )
        filtered = query(
            {
                "filters": [
        ...
        assert traversed["type_group_values"] == []
        ...
        for request in requests:
            ...
            assert (
                invocation["output"]["type_group_values"] == result["type_group_values"]
            )
            assert (
                invocation["output"]["type_group_bindings"]
                == result["type_group_bindings"]
            )
    ...
    print(
        "Retained source type group, common-field traversal and exact Function input replay verified."
    )
