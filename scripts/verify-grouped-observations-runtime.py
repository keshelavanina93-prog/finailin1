"""Prove grouped observation counts over an authentic retained source window."""
...
KEY = "sog-procurement:grouped-observation-chain:v1"
FUNCTION_KEY = "sog-procurement:grouped-observation-counts:v1"
...
def prepare(author, reviewer):
    source_id = canonical_id(
        ...
        "FunctionDefinition",
        "sog-procurement:posting-date-query:v1",
    )
    source = resources.get_resource(author, source_id)["resource"]
    assert source["attributes"]["object_set_id"] == OBJECT_SET
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "docs/development/evidence/nin6-grouped-observations-runtime.json"
        ),
    )
