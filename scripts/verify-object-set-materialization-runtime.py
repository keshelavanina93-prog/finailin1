"""Verify bounded multi-page source materialization and retained temporal extent."""
...
spec = importlib.util.spec_from_file_location(
    "materialization_shared", ROOT / "scripts/verify-transformation-input-runtime.py"
)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
KEY = "sog-source-dates:materialization:v1"
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "docs/development/evidence/nin47-object-set-materialization-runtime.json"
        ),
    )
    ...
    with httpx.Client(
        ...
        assert output["derived_values"] == [] and "group_counts" not in output
        ...
        assert extent["coverage"] == "COMPLETE_BOUNDED_MATERIALIZATION"
        ...
        assert extent["object_count"] == extent["value_count"] == 246
        assert extent["missing_count"] == extent["null_count"] == 0
        for side, boundary in (("earliest", "2025-11-01"), ("latest", "2025-11-30")):
    ...
    save(
        ...
        {
            **previous,
            "phase": "OBJECT_SET_MATERIALIZATION_VERIFIED",
            "result": result,
    ...
    print(
        "Exact 246-object materialization, retained extent and representative evidence verified."
    )
