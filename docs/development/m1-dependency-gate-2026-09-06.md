...
Reviewed definition-conformance certification and exact current-use enforcement are now implemented in commits dedad53/77c5632; see `scoped-certified-consumption.md`. They do not manufacture financial certification. Shared artifact classification and immutable policy assessments are implemented in `artifact-preservation-contract.md`; assessments never authorize disposition, and broader deletion/archival execution remains open. NIN-5 remains gated by NIN-27; its blockers are not removed to advance downstream work.
