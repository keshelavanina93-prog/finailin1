...
## Retained build discovery

Data Builds now discovers prior runs in the operator's exact authorized scope, with bounded keyset pages and direct reopening. The displayed name comes from the exact retained transformation version, not its latest replacement. Completed/failed step counts and publication counts describe retained evidence; the list does not infer live runtime state. Opening a run uses the existing integrity-checked readback and separately queries Temporal.

Migration044 adds the scoped history paging index; readiness requires44. The focused native test covers two history pages, unique run identities, completed/failed counts, other-company exclusion, incomplete cursor refusal and discovery while Function planning is deliberately unavailable. Ruff/mypy, frontend lint/types and production build pass. After sequential API/worker restart, the shared Function and transformation were independently reviewed for the new package; prior-version history remained readable. The actual proxy proof returned the three original builds in two pages (`evidence/nin12-build-history.json`). A new build completed on the updated package with unchanged repeat/read receipts (`evidence/nin12-history-current-runtime.json`).

Authenticated browser discovery displayed four retained builds after the fresh run. Opening the first history entry selected exactly `c57c0377-d419-4681-9985-02a5fd26e5ba`, showed its completed runtime, and opened the three-row first output at its retained 04:53:09 time context. `.finai/artifacts/browser-shots/g8-retained-build-history.png` was captured and viewed. The open action was subsequently adjusted to bring its runtime summary into view after successful readback, rather than leaving the result below the expanded list. After the final rebuild, opening the same history entry focused “Runtime: completed” at visible viewport y=152; `g8-build-history-opened.png` was captured and viewed.
