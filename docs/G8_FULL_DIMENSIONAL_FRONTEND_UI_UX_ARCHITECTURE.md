...
latest implementation head is `0dcef8a`; frontend lint and typecheck pass for the changed surfaces, with learning shadow evaluation, petroleum conservation/lineage, and source-family operational validation represented in the unified workspaces.
...
between governed-promotion eligibility and disabled canonical promotion. Retail cash-register shift-close intake is represented as a wired, partially implemented source family with Z-report identity, fiscal-close review evidence, Store/CashRegister semantic-binding readback and a proposal-only promotion preview. Operations also presents the accepted-resource revenue/volume/COGS/margin bridge with explicit COGS gaps and a movement-to-journal coverage panel with matched and unresolved states. 1C movement-register intake is represented with physical movement grain and evidence-linked proposal preview. Canonical RetailSale promotion remains open. Retained source
