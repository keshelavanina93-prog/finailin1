...
- Canonical schema, semantic and typed-link catalog; 78 explicit platform definitions. Catalog installation creates no enterprise or accounting facts.
...
- An optional, explicitly hypothetical SOCAR Petroleum/Gas reference proposal. It requires review and does not establish real ownership, licence, regulatory or source evidence.
...
Frontend production compilation and backend static type analysis pass. The restarted API serves catalog, graph, context and proposal endpoints against native PostgreSQL; the catalog returns 78 definitions. No automated tests were run for this delivery, as requested. This is code and local runtime evidence, not security certification or release acceptance.
