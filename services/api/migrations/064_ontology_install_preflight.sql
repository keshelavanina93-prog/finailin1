...
-- Read-only diagnostics for the governed platform install.  This exposes only
-- dependent identity metadata; field values remain protected by the existing
-- g8_can_read_version policy.
CREATE OR REPLACE FUNCTION public.g8_hidden_current_dependents(root_resource_id uuid)
RETURNS TABLE(
    resource_id uuid,
    version_id uuid,
    object_type text,
...
BEGIN
    ...
    RETURN QUERY
    SELECT h.resource_id, h.version_id, v.object_type, i.identity_key,
           v.display_name, h.access_entity
    ...
    WHERE d.tenant_id = tenant
      AND d.target_resource_id = root_resource_id
    ...
    ORDER BY i.identity_key, h.version_id;
END $$;

REVOKE ALL ON FUNCTION public.g8_hidden_current_dependents(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.g8_hidden_current_dependents(uuid) TO finai_runtime;
