...
-- Existing scalar observation counts may consume a verified complete materialization.
-- Exact grouped schema authority, stored values and contributor partitions remain unchanged.
CREATE OR REPLACE FUNCTION guard_object_materialization_intent() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 IF intent.plan ? 'materialization' THEN
 ...
 OR grouped->>'coverage' IS DISTINCT FROM
 ...
 OR source_count>(CASE WHEN intent.plan ? 'materialization'
  THEN least((intent.plan->'materialization'->>'max_objects')::integer,
