BEGIN;
CREATE TABLE artifact_retention_evaluations (
 tenant_id uuid NOT NULL, evaluation_id uuid NOT NULL, exact_scope jsonb NOT NULL,
 actor_id text NOT NULL, request_hash text NOT NULL CHECK(request_hash ~ '^[a-f0-9]{64}$'),
 proof_hash text NOT NULL CHECK(proof_hash ~ '^[a-f0-9]{64}$'), payload jsonb NOT NULL,
 recorded_at timestamptz NOT NULL DEFAULT clock_timestamp(), PRIMARY KEY(tenant_id,evaluation_id),
 CHECK(exact_scope->>'tenant_id'=tenant_id::text)
);
ALTER TABLE artifact_retention_evaluations ENABLE ROW LEVEL SECURITY;
ALTER TABLE artifact_retention_evaluations FORCE ROW LEVEL SECURITY;
CREATE POLICY retention_exact_scope ON artifact_retention_evaluations
 ...
 AND exact_scope=current_setting('finai.exact_scope',true)::jsonb);
GRANT SELECT,INSERT ON artifact_retention_evaluations TO finai_runtime;
CREATE TRIGGER immutable_retention_evaluation BEFORE UPDATE OR DELETE OR TRUNCATE
 ON artifact_retention_evaluations FOR EACH STATEMENT EXECUTE FUNCTION deny_evidence_mutation();
CREATE FUNCTION guard_artifact_retention_evaluation() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 IF NEW.payload->'policy' IS NOT NULL AND NEW.payload->'policy'<>'null'::jsonb THEN
  ...
  LOOP
   ...
   IF event.payload->>'target_state'='CERTIFIED' AND
 ...
 RETURN NEW;
END $$;
CREATE TRIGGER retention_evaluation_integrity BEFORE INSERT ON artifact_retention_evaluations
 FOR EACH ROW EXECUTE FUNCTION guard_artifact_retention_evaluation();
